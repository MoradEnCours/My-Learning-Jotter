...
'''
I read over the event.py, main.py, process.py files and looked back at the specification.
From how I interpreted it, it seemed as if  I mostly need to focus on what's given in the des.py
file. Last week you said the idea was to override via inheritance the algorithms to be implemented.

I know there's the despatcher function: this gives control of the CPU over to the process which is selected
by the scheduler. I am wonder if the scheduler is in this case the algorithms being implemented?

  For FCFS:
The overall idea is - the first process which comes is executed to completion, then the one after it
and so on.
I understand that the arrival times of processes are taken into account.
Each process has a CPU burst time which is the slice of time it needs from the CPU to terminate
Based on the time a process arrives and when it executes, I can calculate its waiting time
from (Execution time - Arrival time)
I can then calculate the average waiting time by simply taking the average of the waiting times.
The turnaround time is calculated by looking at the completion time and subtracting from it the
arrival time.

  For SJF:
The idea is to execute processes in the order from lowest CPU burst to highest.

  For SRTF:
Like SJF but has foresight in a sense that it looks at the arrival times of new processes, and
compares their CPU burst time. 

  For Round Robin:
Each process gets a time slice. If the process terminates earlier than the allocated time slice,
the next process executes. If a process is unable to terminate before the time slice finishes,
then it gets pushed to the back of the queue.

*** For SFJ and SRTF, if processes have the same burst time, then FCFS algorithm is used.

Does an event contain the group of processes to be executed? 
And does the current_event mean the current process being executed?
'''