...
public class TrainerSprite implements Runnable {
	...
	/**
	 * Implements the main threaded behaviour -- moves this sprite around the screen,
	 * and engages in battle when appropriate.
	 */
	@Override
	public void run() {
		while (battleReady) {
			try {
				...
				Point newLocation = chooseNextLocation();
			
				// Behave differently depending on if it's a special location
				if (world.isBattleArea(newLocation)) {
					world.enterBattleArea(this);
				} else if (!world.isRestingArea(newLocation)) {
				...
				// If we're interrupted, break the loop
				break;
			}
		}
	}
