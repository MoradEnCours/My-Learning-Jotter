...
public class AddMonsterDialog extends JDialog implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == cancelButton) {
			...
			// Use the appropriate constructor depending on one or two types
			Monster monster;
			if (!type2.isEmpty()) {
				monster = new Monster(name, type1, type2, hp);
			} else {
				monster = new Monster(name, type1, hp);
			}
			...
			for (int i = 0; i < NUM_MOVES; i++) {
				...
				monster.setMove(i, new Move(moveName, moveType, movePower));
			}
			
			// Add the monster to the list model
