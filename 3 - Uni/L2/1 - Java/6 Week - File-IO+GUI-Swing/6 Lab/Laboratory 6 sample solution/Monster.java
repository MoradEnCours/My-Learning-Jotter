...
public class Monster implements TypedItem, Comparable<Monster> {
	...
	/**
	 * Creates a String representation of this Monster suitable for writing to a file.
	 * 
	 * @return A String suitable to be written to a file
	 * @see #parseMonster(List)
	 */
	public List<String> toStringForFile() {
		// TODO students need to write this
		List<String> result = new ArrayList<>();
		String firstLine = name + ";" + hp + ";" + types[0];
		if (types.length == 2) {
			firstLine += ";" + types[1];
		}
		...
		for (Move move : moves) {
			if (move == null) {
				...
				result.add(move.toStringForFile());
			}
		}
		return result;
	}
	...
	/**
	 * Creates a new Monster based on the input specification.
	 * 
	 * @param monsterSpec The specification to parse
	 * @return A Monster based on the specification
	 */
	public static Monster parseMonster (List<String> monsterSpec) {
		// TODO students need to write this

		// First line: name
		String[] values = monsterSpec.get(0).split(";");
		
		// Use correct constructor depending on types
		Monster m;
		if (values.length == 4) {
			m = new Monster(values[0], values[2], values[3], Double.parseDouble(values[1]));
		} else {
			m = new Monster(values[0], values[2], Double.parseDouble(values[1]));
		}

		// Rest of lines: moves
		for (int i = 1; i < monsterSpec.size(); i++) {
			m.setMove(i-1, Move.parseMove(monsterSpec.get(i)));
		}
		return m;
	}
