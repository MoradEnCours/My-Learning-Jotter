...
; Name of program: OrderedListsEXERCUSE
...
;    if q = nil then goto DeleteListLoopEnd
...
;    if p = nil then goto UltimateEnd

;    if found then goto FoundIt

;    if (*p).value /= x then goto AfterIf
...
DeleteWhileLoop
    ...
    load R4,q[R0] ; R1:=(&q)
    load R5,0[R4]	;R2:=(*q).value
 
    cmpeq R6,R5,R4

    jumpf R6,DeleteElsePath[R0] ; if not, go to else path

    load R7,1[R2]	;R7:=(*p).next
    
    lea R10,q[R0] ; R10:=(&q)
    load R3,1[R10] ;R3:=(*q).next

; *p.next = *q.next

    store R3,qpointer[R0]; store *q.next
    load R7,qpointer[R0]; load *q.next into where *p.next is

    jal R13,ReleaseNode[R2] ; call ReleaseNode[q]
    lea R2,0[R0]
    jump DeleteWhileLoop[R0]
...
CmdSearch
    ...
    lea R5,0[R0]; setting found = false
    store R5,found[R0]; store found

;  set p = *p.next again:

    load R1,p[R0]; R1 := p
    load R2,1[R1]; R2 := (*p).next
    store R2,p[R0]; store new p  

SearchWhileLoop


    cmpeq R3,R1,R0; compare p to 0
    jumpt R3,SearchWhileLoopEnd[R0]; if equal, jump to end of loop

    jumpf R5,SearchWhileLoopEnd[R0]; if found is False, jump to end of loop
 
    load R6,x[R0]
    load R4,0[R1]; R4 := *p.value
    cmpeq R7,R4,R6
    jumpf R7,ifAdressofPisnotX[R0], jump to else path if *p.value != x
...
ifAdressofPisnotX
    ...
    load R2,1[R1]; R2 := (*p).next
    jump SearchWhileLoop[R0]; go back to loop start

SearchWhileLoopEnd

    jumpt R5,Xwasfound[R0]

; write no
    lea R8,2[R0]
    load R10,MessageNo[R0]; R10 := MessageNo
    load R3,0[R0]; R3 := *MessageNo.value
    lea R9,2[R0]; R9 := length of Message
    trap R1,R3,R9
    jump SearchEnd[R0]
...
; write yes
    load R10,MessageYes[R0]; R10 := MessageYes
    load R3,0[R0]; R3 := *MessageYes.value
    lea R9,3[R0]; R9 := length of Message
    trap R1,R3,R9
...
PrintWhileLoop
    ...
    cmpeq R3,R1,R0 ; Check whether p = nil
    jumpt R3,CmdDone[R0] ; if so, end loop
 
    load R4,0[R1]
    jal R13,WriteValIntFixedWidth[R4] 

    load R1,p[R0] ; R1 := p ; update p
    load R2,1[R1] ; R2 := (*p).next
    store R2,p[R0] ; store new p

    jump PrintWhileLoop[R0]
